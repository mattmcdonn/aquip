...
enum PoolTreatmentPlanner {
    ...
    static func steps(formData: PoolFormData, analysis: PoolAnalysis) -> [TreatmentStep] {
        ...
        // Water color danger overrides — brown/purple after shock strongly suggests metals
        let dangerousMetalColor = (waterColor == "brown" || waterColor == "purple")
        // Green water without algae selection suggests algae
        ...
        if hasAlgae {
            ...
            // Green water but algae was not selected — treat as likely green algae
            raw.append(StepBuilder(
        ...
        // MARK: Step 6 – CYA low
        // Skip CYA if algae is active — algae treatment takes priority; add CYA after algae is resolved.
        // Exception: if direct sunlight is present and no algae, move CYA up for protection.
        let cyaBeforeCalcium = inDirectSun && !hasAlgae
        if cyaLow && !hasAlgae {
            let cyaVal  = Double(formData.cyanuricAcid) ?? 0
            let target: Double = isSalt ? 70 : 50
        ...
        // MARK: Step 10 – Deferred CYA (only when algae is active — add CYA after algae is resolved)
        if cyaLow && hasAlgae {
            let cyaVal  = Double(formData.cyanuricAcid) ?? 0
            let target: Double = isSalt ? 70 : 50
            let increase = target - cyaVal
            var desc = "CYA is low, but adding stabilizer while algae is active can help algae resist chlorine treatment. Complete algae elimination first. Once the water is clear and free chlorine has been maintained for 24–48 hours with no green or cloudy recurrence, add cyanuric acid."
            if hasVolume && increase > 0 {
                let oz = 13 * (volumeGal / 10_000) * (increase / 10)
                desc += " Estimated dose: \(formatOz(oz)) of cyanuric acid / pool stabilizer to raise CYA by approximately \(Int(increase)) ppm toward a target of \(Int(target)) ppm."
            }
            desc += " CYA dissolves slowly — add it to a skimmer sock or dissolve it before adding. Do not overshoot."
            raw.append(StepBuilder(
    ...
    private static func algaeTypeSteps(
        ...
        let shockNote = "Use \(shockProduct(cyaHigh: cyaHigh, calHigh: calHigh)) and shock to at least 10 ppm free chlorine. Add shock at dusk to prevent UV degradation. Run the pump continuously until the water clears."
